...
```
1. 默认的setting.json是【以图搜图+以文搜图】的配置。如果选择使用该配置，请下载chinese_clip转化为onnx后的模型：
(chinese_clip)[https://github.com/Just-A-Freshman/VimgFind/releases/download/model/chinese_clip_onnx.7z]

2. 如果只希望配置简单的【以图搜图】，将config文件夹中的settingv1.1.json替换原来的setting.json文件夹，然后下载imagenet-b2模型：
(imagenet)[https://github.com/Just-A-Freshman/VimgFind/releases/download/model/imagenet-b2-opti.onnx]
