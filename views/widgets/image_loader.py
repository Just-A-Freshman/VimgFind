...
class ImageLoader:
    __slots__ = ("__executor", "__result_queue")

    def __init__(self) -> None:
        self.__result_queue: queue.Queue[LoaderResult] = queue.Queue()
        self.__executor = ThreadPoolExecutor(max_workers=10, thread_name_prefix="img_loader")

    def add_task(self, item: str, image_path: str, thumbnail_size: int) -> None:
        self.__executor.submit(self._process_one, item, image_path, thumbnail_size)

    def _process_one(self, item: str, image_path: str, thumbnail_size: int) -> None:
        img = image_ops.parse_image_from_path(image_path)
        if img is None:
            self.__result_queue.put(LoaderResult(
        ...
        try:
            ...
            self.__result_queue.put(LoaderResult(item=item, size=(width, height), photo=img, error=""))
