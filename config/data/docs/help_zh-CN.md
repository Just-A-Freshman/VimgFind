...
1. 间断选择：按住键盘左下角的`⌘`（Command）键不松手，然后用鼠标就可以选中多张不同的图片，然后执行对应的右键菜单命令了（macOS 上 `Control`+点击是系统右键，不能用于多选）；
...
**支持。你可以通过拖拽、粘贴，或浏览时多选（`⌘`/`Shift`）来同时输入多张图片进行搜索。**
...
一个模型文件夹准备好后，将其放置在：`~/Library/Application Support/VimgFind/models/`下就可以直接被识别使用了。当然，你也可以先把模型文件夹压缩成`.zip`格式，然后在模型选项卡点击加载本地模型。
...
> ⚠️ **macOS 注意（Safari 自动解压）：**
> Safari 下载压缩包后，会对“只含单个文件/文件夹”的压缩包**自动解压**，导致模型包变成裸的 `model.onnx` 文件或文件夹，模型 ID（文件夹名）信息丢失，无法再通过`加载本地模型`导入。
> 解决方法（任选其一）：
> - 优先使用**应用内下载**（模型选项卡的下载按钮），应用自行保管 zip，不经过浏览器；
> - 关闭 Safari 的自动解压：Safari → 设置 → 通用 → 取消勾选“下载后打开‘安全’的文件”；
> - 改用 Chrome 等其他浏览器或第三方下载工具（默认不会自动解压）。
> 无论哪种方式，导入时请确保选中的是 **.zip 压缩包**，而不是解压后的文件。
...
| 调用自己的 Python 脚本    | `python ~/tools/my_script.py {path} {dir}/result.png`        | 脚本接收两个参数：输入路径和输出路径，在内部自由处理      |

**这些变量是什么意思？**

| 变量      | 代表什么                                                     |
| --------- | ------------------------------------------------------------ |
| `{path}`  | 选中图片的完整路径，比如 `/Users/name/照片/2024/IMG_001.png` |
| `{dir}`   | 图片所在的文件夹                                             |
...
```
复制路径到剪贴板 → python ~/tools/copy_paths.py {paths}
```
