...
在2.5.4版本，排除规则的预览效果是在[索引界面]左侧的图库目录中查看。被排除的图片会用红色删除线标识：

![image-20260923203051483](image/image-20260923203051483.png)

⚠️ **重要说明：** 图库目录的删除线只用于表达排除规则成功作用在了这个文件上，但不代表这个文件已经从索引中删除了。它所承担的只是一个预览功能，并不会真正对索引造成任何影响。如果你需要将红色删除线标识的图片真正移除出索引，你需要点击“清理排除图片”按钮。
...
![image-20260923203549109](image/image-20260923203549109.png)
...
> **问题：**
> 我一般只在工作时偶尔用一下这个工具，每次用完就关掉。但下次重新打开总要等一会儿——得重新读索引、重新加载模型。能不能关掉窗口但不退出程序？

**可以。在 通用设置 → 常规 里把「关闭行为」选成「收缩到托盘」。**

| 选项 | 点窗口右上角 X 之后 | 适合谁 |
| --- | --- | --- |
| 收缩到托盘 | 窗口消失，程序留在系统托盘（任务栏右下角）继续运行，双击托盘图标随时唤回 | 想“随用随开”、不想等重新启动的人 |
| 直接关闭 | 程序完全退出，内存全部释放 | 用完就彻底结束，不介意下次重新启动的人 |
| 每次询问 | 每次点 X 都弹窗问一次，弹窗里可以勾选“记住我的选择” | 还没想好，或者不同场合需求不同的人 |

**托盘的用法：**

| 操作 | 效果 |
| --- | --- |
| 单机托盘图标 / 右键 → 显示主窗口 | 窗口恢复（最大化状态也会保留） |
| 右键 → 退出 | 程序真正退出（会正常保存索引） |

**藏到托盘之后会发生什么：**

| 行为 | 说明 |
| --- | --- |
| 索引保留在内存里 | 唤回后搜索依旧是即时的，不需要重新读索引 |
| 模型内存在 30 秒后释放 | 一直藏在托盘不用的话，30 秒后把模型从内存里放掉，此时内存将主要由索引的大小决定。关于是否需要更激进一些，甚至将索引内存也释放掉——实现并不太容易，因此该想法暂时搁置。 |
| 空闲时仍会自动更新索引 | 按“自动更新索引”的设置继续工作，更新结束后同样会释放模型内存 |
| 重新使用时会稍慢一下 | 模型被释放后，第一次搜索需要重新加载模型（几百毫秒到一两秒），之后就恢复正常 |

> **注意：** “释放模型内存”只在**藏到托盘**时发生。如果你只是把窗口**最小化**到任务栏，程序会认为你马上还要用，不会释放内存。
