...
> **问题：**
> 我写了 `*.png` 把所有 PNG 都排除了，但文件夹里有一个 `重要图表.png` 我想保留。怎么办？
...
2. 以下情况情况 `!` 不起作用：
    `!` 取反规则**只对普通的文件名模式生效**，以下两种情况无法用 `!` 拉回：
...
你什么都不用管，命令该怎么写还怎么写。选中 50 张图片，工具会自动把你写的命令执行 50 次，每次都会把命令中的变量替换成当前那一张图的信息。
...
命令只执行一次，但你需要使用 `{paths}`（注意有个 **s**）来接收所有文件的列表。
...
| 直接在命令里写                     | 封装成脚本                           |
| ---------------------------------- | ------------------------------------ |
| 受工具语法限制，一行内难以完成     | 使用完整的编程语言，任何逻辑都能实现 |
| 对特殊字符（引号、管道等）处理棘手 | 脚本内部可以自由处理任意复杂情况     |
| 换一台电脑可能要调整               | 脚本可复用、可分享、可版本管理       |
...
1. 提议新功能，你应该详细解释为什么你需要这个功能，现有的操作策略是什么；
2. 反馈BUG，明确解释其复现流程；
3. 对某项原有功能 / UI 的改进想法；
