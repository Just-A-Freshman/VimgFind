...
def create_tokenizer(model_dir: str) -> BaseTokenizer | None:
    ...
    if vocab_txt.exists():
        return BertWordPieceTokenizer(str(vocab_txt))
    ...
    if merges_txt.exists() and vocab_json.exists():
        return CLIPBpeTokenizer(str(vocab_json), str(merges_txt))
...
def _create_from_tokenizer_json(path: str) -> CLIPBpeTokenizer | None:
    ...
    try:
        ...
        tokenizer = CLIPBpeTokenizer(vpath, mpath)
        return tokenizer
    except Exception:
        return None
